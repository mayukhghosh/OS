...
public class Main {
	...
	static boolean random_read=false;
	static int[]numbers;
	...
	public static void scheduler()throws Exception
	{
		...
		while(true)
		{
			/*if(cycle==132)
			{
				System.out.println("Process 2: "+processes[2].r+" "+processes[2].T+" "+processes[2].t);
				System.out.println("Process 4: "+processes[4].r+" "+processes[4].T+" "+processes[4].t);
			}*/
			for(int i=0;i<processes.length;i++)
			{
			...
			Process dummy=new Process(-1,1,1,1);
			dummy.r=9999999;
			dummy.position=-1;
			HPRN.enqueue(dummy);
			HPRN.dequeue();
